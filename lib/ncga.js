...
let ncga = {
	...
	urls: {
		...
		imageUrl: (mapIdentifier) => {
			return new Promise((resolve, reject) => ncga.auth.getKey()
				.then(key => resolve(`${ncga.urls.mapBase}?map=${mapIdentifier}&key=${encodeURIComponent(key)}`))
				.catch(err => reject(err)));
		}
	}
};
